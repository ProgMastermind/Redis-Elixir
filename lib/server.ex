...
require Logger
  ...
  defp serve(client, config) do
    is_master = is_master_connection?(client, config)
    serve_loop(client, config, is_master, "")
  end
  ...
  defp serve_loop(client, config, is_master, buffer) do
    case read_line(client) do
      {:ok, data} ->
        new_buffer = buffer <> data
        case Server.Protocol.parse_multi(new_buffer, 1) do
          {:ok, [command], rest} ->
            process_command(command, client, config, is_master)
            serve_loop(client, config, is_master, rest)
          {:continuation, _} ->
            serve_loop(client, config, is_master, new_buffer)
        end
      {:error, :closed} ->
        :ok
      {:error, reason} ->
  ...
  defp process_command(command, client, config, is_master) do
    IO.puts("Received command: #{inspect(command)}")  # Debug line
    handle_command(command, client, config, is_master)
  end

  defp handle_command(parsed_data, client, config, is_master) do
    case parsed_data do
      [command | args] ->
        execute_command_with_config(String.upcase(to_string(command)), args, client, config, is_master)
      _ ->
        unless is_master do
          write_line("-ERR Invalid command format\r\n", client)
        end
  ...
  defp execute_command_with_config(command, args, client, config, is_master) do
    case command do
      "INFO" when args == ["replication"] ->
        handle_info_replication(client, config)
      _ ->
        execute_command(command, args, client, is_master)
    end
  end
  ...
  defp execute_command("REPLCONF", args, client, _is_master) do
  ...
  defp execute_command("PSYNC", _args, client, _is_master) do
    try do
  ...
  defp execute_command("ECHO", [message], client, _is_master) do
  ...
  # defp execute_command("PING", [], client, is_master) do
  #   write_line("+PONG\r\n", client)
  # end

  # defp execute_command(command, _args, client, is_master) do
  #   write_line("-ERR Unknown command '#{command}'\r\n", client)
  # end

  defp execute_command("SET", [key, value | rest], client, is_master) do
    try do
      case rest do
        [command, time] ->
          command = String.upcase(to_string(command))
  ...
  defp execute_command("GET", [key], client, is_master) do
    IO.puts("Executing GET command for key: #{key}")
    case Server.Store.get_value_or_false(key) do
      {:ok, value} ->
        IO.puts("Value found: #{value}")
